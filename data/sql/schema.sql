...
CREATE TABLE itemMaster (id BIGINT AUTO_INCREMENT, code VARCHAR(10) NOT NULL, name VARCHAR(64) NOT NULL, unit VARCHAR(32) NOT NULL, description VARCHAR(255), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE itemtyp (id BIGINT AUTO_INCREMENT, name VARCHAR(64) NOT NULL, PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE job (id BIGINT AUTO_INCREMENT, contact_person VARCHAR(255), contact_info VARCHAR(255), job_type_id BIGINT NOT NULL, end DATETIME NOT NULL, start DATETIME, timeed DATETIME, description LONGTEXT, timeinterval TINYINT, job_state_id BIGINT NOT NULL, store_id BIGINT NOT NULL, created_at DATETIME NOT NULL, created_from BIGINT, updated_at DATETIME NOT NULL, updated_from BIGINT, INDEX store_id_idx (store_id), INDEX job_state_id_idx (job_state_id), INDEX job_type_id_idx (job_type_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE jobchangelog (id BIGINT AUTO_INCREMENT, job_id BIGINT NOT NULL, user_id BIGINT NOT NULL, action TEXT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX job_id_idx (job_id), INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE task (id BIGINT AUTO_INCREMENT, start DATETIME, end DATETIME, scheduled TINYINT(1), break BIGINT, overtime DECIMAL(18, 2), info LONGTEXT, approach BIGINT, job_id BIGINT, task_type_id BIGINT DEFAULT 1, correction_time DECIMAL(18, 2), correction_info TEXT, created_at DATETIME NOT NULL, created_from BIGINT, updated_at DATETIME NOT NULL, updated_from BIGINT, INDEX job_id_idx (job_id), INDEX task_type_id_idx (task_type_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE taskchangelog (id BIGINT AUTO_INCREMENT, user_id BIGINT NOT NULL, action TEXT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, task_id BIGINT, INDEX task_id_idx (task_id), INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE jobchangelog ADD CONSTRAINT jobchangelog_user_id_sf_guard_user_id FOREIGN KEY (user_id) REFERENCES sf_guard_user(id);
ALTER TABLE jobchangelog ADD CONSTRAINT jobchangelog_job_id_job_id FOREIGN KEY (job_id) REFERENCES job(id);
...
ALTER TABLE job_user ADD CONSTRAINT job_user_job_id_job_id FOREIGN KEY (job_id) REFERENCES job(id);
...
ALTER TABLE taskchangelog ADD CONSTRAINT taskchangelog_user_id_sf_guard_user_id FOREIGN KEY (user_id) REFERENCES sf_guard_user(id);
ALTER TABLE taskchangelog ADD CONSTRAINT taskchangelog_task_id_task_id FOREIGN KEY (task_id) REFERENCES task(id);
ALTER TABLE taskposition ADD CONSTRAINT taskposition_item_id_item_id FOREIGN KEY (item_id) REFERENCES item(id);
ALTER TABLE task_user ADD CONSTRAINT task_user_user_id_sf_guard_user_id FOREIGN KEY (user_id) REFERENCES sf_guard_user(id) ON UPDATE RESTRICT ON DELETE RESTRICT;
